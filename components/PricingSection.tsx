        ...
        {PLANS.map((plan) => (
          <div
            ...
            className={`flex flex-col rounded-2xl border p-6 md:p-7 ${
              plan.highlighted
                ? "border-route-amber bg-ink-900 text-paper shadow-lg md:-translate-y-2"
                : "border-ink-900/15 bg-white/60 text-ink-900"
            }`}
          >
            {plan.highlighted && (
              <span className="mb-3 inline-block w-fit rounded-full bg-route-amber px-3 py-1 text-xs font-mono text-ink-900">
                Популярный выбор
              </span>
            )}
            <h3 className="font-display text-xl mb-1">{plan.name}</h3>
            <p className={`text-sm mb-4 ${plan.highlighted ? "text-paper/70" : "text-ink-900/60"}`}>
              {plan.description}
            </p>
            <div className="mb-6">
              <span className="font-display text-3xl">{plan.price}</span>
              <span className={plan.highlighted ? "text-paper/60" : "text-ink-900/50"}>
                {plan.period}
              </span>
            </div>
            <ul className="mb-6 flex-1 space-y-2.5">
              {plan.features.map((f, i) => (
                <li key={i} className="flex gap-2 text-sm">
                  <span className={plan.highlighted ? "text-route-amber" : "text-route-teal"}>✓</span>
                  <span>{f}</span>
                </li>
              ))}
            </ul>
            <button
              ...
              className={`w-full rounded-full px-5 py-3 text-sm font-medium transition-colors ${
                plan.highlighted
                  ? "bg-route-amber text-ink-900 hover:bg-route-amber/90"
            ...
            {clicked === plan.id && (
              <p className="mt-3 text-xs font-mono text-route-teal">
